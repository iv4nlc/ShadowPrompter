...
const aiToggle = document.getElementById('ai-backend-toggle');
...
function setBackendToggle(mode) {
    currentBackend = mode;
    const isGroq = mode === 'groq';
    aiToggle.setAttribute('aria-pressed', isGroq);
    if (isGroq) {
        document.querySelector('.ai-toggle-label.left').style.color = '';
        document.querySelector('.ai-toggle-label.right').style.color = 'var(--toggle-active-text)';
    } else {
        document.querySelector('.ai-toggle-label.left').style.color = 'var(--toggle-active-text)';
        document.querySelector('.ai-toggle-label.right').style.color = '';
    }
}
...
socket.on('backend_mode', (mode) => {
    setBackendToggle(mode);
     if (mode === 'ollama') {
        const modal = document.getElementById('groq-config-modal');
...
aiToggle.addEventListener('click', () => {
    const newMode = currentBackend === 'ollama' ? 'groq' : 'ollama';
    socket.emit('set_backend', { backend: newMode });
});
...
// ---------- Groq Config Modal ----------
function createGroqModal() {
  if (document.getElementById('groq-config-modal')) return;

  const modalHtml = `
    <div id="groq-config-modal" class="modal-overlay" style="display:none;">
      <div class="modal-content glass-panel">
        <h3>Groq Configuration</h3>
        <label>API Key</label>
        <input type="password" id="groq-api-key" class="action-input" placeholder="gsk_...">
        <label>Model</label>
        <input type="text" id="groq-model" class="action-input" value="llama-3.3-70b-versatile">
        <div class="modal-actions">
          <button id="save-groq-config" class="primary-btn">Save</button>
          <button id="cancel-groq-config" class="cancel-btn">Cancel</button>
        </div>
      </div>
    </div>`;
  ...
  style.textContent = `
    ...
    .modal-content { padding: 24px; min-width: 400px; }
    .modal-actions { display:flex; gap:12px; margin-top:20px; justify-content:flex-end; }
    .modal-content label { display:block; margin-top:12px; font-weight:600; }
    .modal-content input { width:100%; margin-top:6px; }
    .cancel-btn {
    display: inline-flex;
    align-items: center;
  ...
  const overlay = document.getElementById('groq-config-modal');
  const apiKeyInput = document.getElementById('groq-api-key');
  const modelInput = document.getElementById('groq-model');

  document.getElementById('save-groq-config').addEventListener('click', () => {
    const apiKey = apiKeyInput.value.trim();
    const model = modelInput.value.trim() || 'llama-3.3-70b-versatile';
    if (!apiKey) {
      addLogEntry('API Key is required', 'error');
      return;
    }
    socket.emit('save_groq_config', { api_key: apiKey, model: model });
    overlay.style.display = 'none';
  });

  document.getElementById('cancel-groq-config').addEventListener('click', () => {
    overlay.style.display = 'none';
  });
...
socket.on('groq_config_required', () => {
  createGroqModal();
  document.getElementById('groq-config-modal').style.display = 'flex';
  addLogEntry('Groq requires API key. Please configure.', 'warning');
});

function addGroqSettingsButton() {
  const wrapper = document.querySelector('.ai-toggle-wrapper');
  if (!wrapper || document.getElementById('groq-settings-btn')) return;
  const btn = document.createElement('button');
  btn.id = 'groq-settings-btn';
  btn.className = 'icon-btn subtle';
  btn.style.width = '30px';
  btn.style.height = '30px';
  btn.innerHTML = '<i class="fa-solid fa-gear"></i>';
  btn.title = 'Configure Groq';
