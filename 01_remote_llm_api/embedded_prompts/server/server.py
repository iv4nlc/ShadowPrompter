...
def load_groq_config():
    ...
    if 'api_key' not in config or 'model' not in config:
        raise ValueError("config.json must contain 'api_key' and 'model'")
    return config
...
class TCPServer:
    def __init__(self, notify_callback, command_output_callback, groq_config):
    ...
    def _save_command_history(self, client_id, output):
        ...
        try:
            for line in reader:
                ...
                if line == "[SYSTEM INFO]":
                    ...
                    in_system_info = False
                    session_key_line = reader.readline().rstrip('\n')
                    if not session_key_line.startswith("SESSION_KEY:"):
                        self.notify_callback('log', f'Missing or invalid session key from {addr}')
                        break
                    enc_key_b64 = session_key_line[len("SESSION_KEY:"):]
                    try:
                    ...
                    ai_config_json = json.dumps(self.groq_config)
                    conn.sendall(f"AI_CONFIG:{ai_config_json}\n".encode('utf-8'))
                    ...
                    self.notify_callback('log', f'Info: OS={info.os}, Host={info.hostname}, User={info.user}')
                    continue
                ...
                if client_id:
                    try:
                        ...
                        if encrypted_response:
                            ...
                            self.command_output_callback(client_id, "", output)
                            self._save_command_history(client_id, output)
                    except Exception as e:
...
def handle_start_server():
    ...
    if tcp_server is None:
        ...
        tcp_server = TCPServer(notify_callback=notify_web, command_output_callback=command_output_to_web, groq_config=groq_config)
